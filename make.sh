...
setup_vars() {
    IMAGE_PREFIX=${IMAGE_PREFIX:-"nanochain"}
...
docker_clean_builds() {
    echo "> clean: nanochain build images"
    _docker_clean "org.nanochain.name=nanochain"
}

docker_clean_all() {
    echo "> clean: nanochain* images"
    _docker_clean "org.nanochain.name"
}
